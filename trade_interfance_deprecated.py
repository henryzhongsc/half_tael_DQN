...
def verify_account(decorated):
    def inner(*args):
        try:
            decorated(*args)
        except TI_Account_Exception as e:
            sys.exit("{} is raised as: {}".format(type(e), e))
    return inner
...
class Trade_Interface:
    ...
    def __init__(self, _account_name, _record, _currency_pair, _currency_A_balance, _currency_B_balance):
        ...
        if isinstance(_record, str):
            if os.path.isfile(_record):
                df = pd.read_csv(_record)
            else:
                raise TI_Account_Exception("Invalid _record input: CSV file not found at {}".format(_record))
        elif isinstance(_record, pd.DataFrame):
            df = _record
        else:
            raise TI_Account_Exception("Invalid _record input: {} is not a DataFrame but a {}".format(_record, type(record)))
        self.record = df
